...
public class DecoderMessageSplit {
	
	//Decode when we splitted into two parts when length of the first part is equal to length of the second part
	public String DecodeSplitHalf(String Message)
	{
	...
	//Decode when we splitted into two parts when length of the first part is bigger then length of the second part
	public String DecodeSplitLeftBigger (String Message)
	{
		int checkAdditionalSymbol = 0;
		String additionalSymbol = null;
		int Length = Message.length();
		...
		else
		{
			if (Length < 4)
			{
				Message = Message.substring(0, 1) + Message.substring(1);
				return Message;
			}
		}
	...
	//Decode when we splitted into two parts when length of the first part is less then length of the second part
	public String DecodeSplitRigthBigger (String Message)
	{
		int checkAdditionalSymbol = 0;
		String additionalSymbol = null;
		int Length = Message.length();
